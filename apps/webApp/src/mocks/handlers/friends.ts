...
export const friendsHandlers = [
  ...
  // 친구 요청 리스트
  http.get<object, object, FriendRequestResponse[]>(`${BASE_URL}/requests`, async () => {
  ...
  // 친구 승인
  http.post<object, object>(`${BASE_URL}/requests/:requestId/accept`, async () => {
    return HttpResponse.json(
      { status: 200 },
    );
  }),
  ...
  // 친구 거절
  http.post<object, object>(`${BASE_URL}/requests/:requestId/reject`, async () => {
    return HttpResponse.json(
      { status: 200 },
    );
  }),
